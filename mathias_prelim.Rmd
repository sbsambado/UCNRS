...
ggsave("TickCount_ReserveMonth_rough_v2.jpeg", dpi = 320)
```
...
mathias fire proposal
```{r}

tick_counts_sp_f <- tick_counts_sp[which(tick_counts_sp$reserve == "Big Creek" |
         tick_counts_sp$reserve == "Hastings" |
         tick_counts_sp$reserve == "McLaughlin"),]


tick_counts_sp_f$guess <- factor(tick_counts_sp_f$guess,
                          levels = c("J","A"))
ggplot(tick_counts_sp_f, aes(x = reserve, y = count, fill = guess)) +
  geom_bar(stat = "identity") +
  facet_wrap(~month)+
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, vjust = .5))  +
  theme(strip.text.x = element_text(face = 'bold'),strip.background = element_rect(fill = 'gray95')) +
  scale_fill_manual(values = c("sienna3","slateblue4"))+
  #scale_fill_brewer(palette = "Dark2")+
  labs(x = "Reserve", y = "Tick Count", fill = "Lifestage") 
#ggsave("MathiasFireLifestageMonth.png", dpi = 320)
```
...
density <- smol_juv %>%#aggregate.smol.MWMA %>%
  filter(region == "Midwest")

tick_counts %>%
  ggplot(aes(x = month, group_by = reserve,fill = reserve)) +
...
mu <- ddply(density, "sexOrAge", summarise, grp.mean = mean(week))
head(mu)


tick_counts %>%
  ggplot(aes(x = month, fill = reserve)) +
  geom_density(alpha = 0.3) +
  #scale_fill_manual(values = c("#69b3a2", "#404080")) +
  theme_classic() +
  labs(fill = "", x = "Month", y = "Density", title = "Midwest Tick Phenology")+
  theme(legend.position = "bottom") +
  #geom_vline(data = mu, aes(xintercept = grp.mean, color = sexOrAge), linetype = "dashed") +
  guides(color = FALSE) +
...
try to make liz long so each juvenile tick has it's own column
```{r}


data.frame(liz[rep(seq_len(dim(liz)[1]), liz$count), 2, drop = FALSE], row.names=NULL)
...
# proportions
```{r}
# tot_count <- aggregate(count ~ lifestage + reserve, data = liz_tidy, FUN = sum)
# 
# total_count <- merge(liz_tidy, tot_count, by = c("lifestage","reserve"))
# 
# 
# total_count$proportions <- total_count$count.x/total_count$count.y
# 
# ggplot(total_count, aes(x = date, y = proportions)) +
#   geom_point() +
#   facet_wrap(~location)