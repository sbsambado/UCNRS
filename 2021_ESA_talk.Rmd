...
liz_tidy %>% filter(reserve == "Rancho Marino") %>% select(round)
```
...
```{r}
unique(liz_tidy$reserve)

liz_main <- liz_tidy[which(liz_tidy$reserve == "Fort Ord" |
...
committee meeting graph
```{r}
ggplot(liz_main, aes(x = round, y = log(count + 1)), color = reserve) +
  geom_boxplot(fill = "grey85")+
  geom_jitter( aes(color = reserve),alpha = .6, width = .3) +
  scale_color_manual(values = cal_palette("figmtn", n = 4, type = "continuous"))+
  labs(x = "Sampling Round",y =  "log(tick counts)\n per lizard",color = "Reserve") +
  theme_bw() +
  facet_wrap(~location)+
  theme(strip.text.x = element_text(face = 'bold'),strip.background = element_rect(fill = 'gray95')) +
  theme(axis.title.y =  element_text(face = 'bold',size = 11, vjust = .5)) +
  theme(axis.title.x =  element_text(face = 'bold',size = 11, vjust = .5)) +
  theme(legend.position =  "bottom", legend.title = element_text(size = 8), legend.text =  element_text(size = 6), legend.key.size = unit(0, 'lines'))
#ggsave("LizLocation.png", dpi = 320)
...
ggplot(liz_main, aes(x = round, y = log(count + 1)), color = reserve) +
  ...
  geom_jitter( aes(color = reserve),alpha = .6, width = .3) +
  scale_color_manual(values = cal_palette("figmtn", n = 4, type = "continuous"))+
  labs(x = "Sampling Round",y =  "log(tick counts) per lizard",color = "Reserve") +
  theme_bw() +
  facet_grid(lifestage~location)+
  theme(strip.text.x = element_text(face = 'bold'),strip.background = element_rect(fill = 'gray95')) +
  theme(axis.title.y =  element_text(face = 'bold',size = 11, vjust = .5)) +
  theme(axis.title.x =  element_text(face = 'bold',size = 11, vjust = .5)) +
  theme(legend.position =  "bottom", legend.title = element_text(size = 8), legend.text =  element_text(size = 6), legend.key.size = unit(0, 'lines'))
#ggsave("LizLocationGrid.png", dpi = 320)
...
liz_main_fo <-liz_tidy %>%
  filter(reserve == "Fort Ord") 

ggplot(data=liz_main_fo, aes(x= log(count+1), group=lifestage, fill=lifestage)) +
    geom_density(adjust=1.5, alpha=.4) +
    theme_ipsum()+
  facet_wrap(~round, scale = "free_y") +
# larval red, nymphal blue
  guides(fill = FALSE)


liz_main_se <-liz_tidy %>%
  filter(reserve == "Sedgwick") 

ggplot(data=liz_main_se, aes(x= log(count+1), group=lifestage, fill=lifestage)) +
    geom_density(adjust=1.5, alpha=.4) +
    theme_ipsum()+
  facet_wrap(~round, scale = "free_y", nrow = 1) +
# larval red, nymphal blue
  guides(fill = FALSE)
...
liz_main$round <- as.numeric(liz_main$round)
aggregate.smol.MWMA <- aggregate(count ~ reserve + round + lifestage, data=liz_main, FUN=sum)

fo <- liz_tidy %>% 
  filter(reserve == "Fort Ord")
ggplot(fo, aes(x=as.numeric(log(count+1)), fill=lifestage)) +
...
ggplot(ht, aes(x=as.numeric(log(count+1)), fill=lifestage)) +
  geom_density(alpha=0.6) +
  theme_bw()+
  facet_wrap(~round)+
    guides(fill = FALSE, color = FALSE)
...
rm <- liz_tidy %>% 
  filter(reserve == "Rancho Marino")
ggplot(rm, aes(x=as.numeric(log(count+1)), fill=lifestage)) +
  geom_density(alpha=0.6) +
  theme_bw()+
  facet_wrap(~round)+
    guides(fill = FALSE, color = FALSE)
...
sd <- liz_tidy %>% 
  filter(reserve == "Sedgwick")
ggplot(sd, aes(x=as.numeric(log(count+1)), fill=lifestage)) +
  geom_density(alpha=0.6) +
  theme_bw()+
  facet_wrap(~round)+
    guides(fill = FALSE, color = FALSE)
...
sr <- liz_tidy %>% 
  filter(reserve == "Stunt Ranch")
ggplot(sr, aes(x=as.numeric(log(count+1)), fill=lifestage)) +
  geom_density(alpha=0.6) +
  theme_bw()+
  facet_wrap(~round)+
    guides(fill = FALSE, color = FALSE)
...
```{r}
ggplot(liz_main, aes(as.numeric(round), count, color = lifestage, fill = lifestage)) +
  geom_histogram(stat = "identity") +
  facet_wrap(~reserve, scales = "free_y", nrow = 2)  +
...
co<- ggplot(coast, aes(as.numeric(round), count, color = lifestage, fill = lifestage)) +
  geom_histogram(stat = "identity") +
  facet_wrap(~reserve, scales = "free_y", ncol = 1)  +
  labs(x = "Month", y = "Count") +
  theme_bw() +
  theme(strip.background = element_rect(fill = "grey95"),
        strip.text = element_text(size=7, face = 'bold'),
...
inlandd<-ggplot(inland, aes(as.numeric(round), count, color = lifestage, fill = lifestage)) +
  geom_histogram(stat = "identity") +
  facet_wrap(~reserve, scales = "free_y", ncol = 1)  +
  labs(x = "Month", y = "Count") +
  theme_bw() +
...
n<-ggplot(region_n, aes(as.numeric(round), count, color = lifestage, fill = lifestage)) +
  geom_histogram(stat = "identity") +
  facet_wrap(~reserve, scales = "free_y", ncol = 1)  +
  labs(x = "Month", y = "Count") +
  theme_bw() +
...
c<-ggplot(region_c, aes(as.numeric(round), count, color = lifestage, fill = lifestage)) +
  geom_histogram(stat = "identity") +
  facet_wrap(~reserve, scales = "free_y", ncol = 1)  +
  labs(x = "Month", y = "Count") +
  theme_bw() +
...
s<-ggplot(region_s, aes(as.numeric(round), count, color = lifestage, fill = lifestage)) +
  geom_histogram(stat = "identity") +
  facet_wrap(~reserve, scales = "free_y", ncol = 1)  +
  labs(x = "Month", y = "Count") +
  theme_bw() +
